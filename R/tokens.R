# tokens.R
# functions: checkTokenCommon, getTokenName, getTokenId, token

library("ghql")

checkTokenCommon <- function(x) {

  #' checkTokenCommon
  #'
  #' @description Checks against common tokenIds and token names. If a tokenId
  #' is supplied and it matches a common one, it returns the tokenId, and
  #' vice versa.
  #' @param x character
  #' @usage checkTokenCommon("string")
  #' @return If a tokenId is supplied and it matches a common one, it returns
  #' the tokenId, and vice versa. If no matches, returns NULL.
  #' @details The inputs can be anything that can be input into
  #' the paste function.
  #' @note Expected use is as auxiliary function to token().
  #' @examples
  #' checkTokenCommon("03faf2cb329f2e90d6d23b58d91bbb6c046aa143261cc21f52fbe2824bfcbf04")
  #' checkTokenCommon("SigUSD")
  ...
  df <- data.frame(tokenId = c("03faf2cb329f2e90d6d23b58d91bbb6c046aa143261cc21f52fbe2824bfcbf04",
                   ...
                   tokenName = c("SigUSD", "SigRSV", "COMET", "Erdoge", "kushti",
                                 ...
                                 "PEPERG", "Gyros", "$Lambo", "QUACKS", "Gluon",
                                 "GORT"))
...
getTokenName <- function(tokenId) {
  ...
  #' getTokenName
  #'
  #' @description Uses a GraphQL query to retrieve a token name from a given
  #' tokenIds.
  #' @param tokenId character
  #' @usage getTokenName("tokenId")
  #' @return If the tokenId is supplied, it returns the token name. If there are
  #' no matches, the program returns NULL.
  #' @details The inputs can be anything that can be input into
  #' the paste function.
  #' @note Expected use is as auxiliary function to token().
  #' @examples
  #' getTokenName("03faf2cb329f2e90d6d23b58d91bbb6c046aa143261cc21f52fbe2824bfcbf04")
...
getTokenId <- function(name = NULL,
  ...
  #' @param tokenId character
  #' @usage getTokenName("tokenId")
  #' @return If a token name is supplied, it returns the tokenId. If there are
  #' no matches, the program returns NULL.
  #' @details The inputs can be anything that can be input into
  #' the paste function.
  #' @note Expected use is as auxiliary function to token().
  ...
  qry <- Query$new()
  qry$query('gettokeninfo', 'query getTokenInfo($name: String!){
  tokens(name: $name) {
    tokenId
  }
}')
  ...
  con <- GraphqlClient$new(gqlURL)
  res <- con$exec(qry$queries$gettokeninfo, name)
  df <- as.data.frame(jsonlite::fromJSON(res))
  if (nrow(df) != 0) {
    return(as.vector(df))
  } else {
...
token <- function(x) {
  ...
  #' token
  #'
  #' @description This program runs three other functions: checkTokenCommon,
  #' getTokenId, getTokenName.    GraphQL query to retrieve a token name from a given
  #' tokenIds.
  #' @param tokenId character
  #' @usage getTokenName("tokenId")
  #' @return If the tokenId is supplied, it returns the token name. If there are
  #' no matches, the program returns NULL.
  #' @details checkTokenCommon contains a list of common tokens
  #' that checks the parameters against both the tokenID and token name. If
  #' the tokenId is a match, it returns the token name, and vice versa.
  #' Otherwise, the length of the string is checked with a GraphQl query. If it
  #' is 64 characters, then program assumes that it is a tokenId and runs
  #' the getTokenName function. If not, it then checks to see if there is an
  #' exact match for the string in the names of tokens on the Ergo blockchain
  #' with getTokenId and returns tokenId if there is a match.
  #' @examples
  #' token("03faf2cb329f2e90d6d23b58d91bbb6c046aa143261cc21f52fbe2824bfcbf04")
  #' token("SigUSD")

  common <- checkTokenCommon(x)
